(() => {
  ...
  const SCORE_CACHE_KEY='football-score-cache-v2';
  const SCORE_CACHE_MAX_AGE_MS=12*60*60*1000;
  ...
  function hydrateScoreCache(){
    try{
      const cached=JSON.parse(localStorage.getItem(SCORE_CACHE_KEY)||'null');
      if(!cached?.savedAt||!cached?.scores)return false;
      const age=Date.now()-new Date(cached.savedAt).getTime();
      if(!Number.isFinite(age)||age<0||age>SCORE_CACHE_MAX_AGE_MS)return false;
      state.scores=cached.scores;
      state.lastRefresh=new Date(cached.savedAt);
      return true;
    }catch{return false}
  }
  function persistScoreCache(){
    try{localStorage.setItem(SCORE_CACHE_KEY,JSON.stringify({savedAt:new Date().toISOString(),scores:state.scores}))}catch{}
  }
  B.persistScoreCache=persistScoreCache;
  ...
  async function fetchLeagueScores(key,league){
    ...
    // Direct-event results are authoritative. Never let a slower bulk scoreboard
    // response overwrite a newer exact-event snapshot.
    const dedup=new Map();
  ...
  async function refreshScores(){
    ...
    persistScoreCache();
    $('#lastRefresh').textContent=`Last refresh: ${formatStamp(state.lastRefresh.toISOString())}`;
  ...
  function renderLoading(){
    $('#combinedSummary').innerHTML='<div class="panel combined-card"><div><div class="combined-title">Loading live scores</div><div class="combined-note">Syncing the latest ESPN game state before calculating the dashboard.</div></div></div>';
    $('#siteUpdated').textContent=`Site updated: ${formatStamp(CFG.lastSiteUpdate)}`;
  }
  B.renderAll=renderAll;
  B.refreshScores=refreshScores;
  ...
  const hydrated=hydrateScoreCache();
  if(hydrated){
    $('#lastRefresh').textContent=`Last refresh: ${formatStamp(state.lastRefresh.toISOString())} · cached`;
    renderAll();
  }else{
    renderLoading();
  }
  refreshScores();
  setInterval(refreshScores,Math.max(30,Number(CFG.refreshSeconds)||60)*1000);
})();
